...
class DataCollatorForCompletionOnlyLM(ts.DataCollatorForLanguageModeling):
    """
    ...
    def torch_call(self, examples):
        ...
        if self.instruction_template is None:
            for i in range(len(examples)):
                ...
                # print(np.where(batch["labels"][i] == self.response_token_ids[0]))
                # print(np.where(batch["labels"][i] == self.response_token_ids[0])[0])
                # print()

                for idx in np.where(batch["labels"][i] == self.response_token_ids[0])[0]:
                    # `response_token_ids` is `'### Response:\n'`, here we are just making sure that the token IDs match
                    # try:
                    #     print(self.tokenizer.decode(batch["labels"][i][idx : idx + len(self.response_token_ids)].tolist()))
                    # except:
                    #     pass
